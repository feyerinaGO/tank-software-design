...
public class RandomTankBotCommandGenerator implements CommandGenerator {
    ...
    public Collection<Command> generateCommands() {
        ...
        for (DynamicObject dynamic : level.dynamicObjects) {
            if (dynamic.position.getType().equals(TypeGameObjects.ENEMY)) {
                ...
                if (getRandomShoot()) {
                    commandList.add(new ShootCommand(dynamic, level));
                }
            }
        }
        return commandList;
    ...
    private Direction getRandomDirection(DynamicObject dynamicObject) {
    ...
    private boolean getRandomShoot() {
        int randInt = ThreadLocalRandom.current().nextInt(1, 2);
        return randInt == 1;
    }
}
